...
export function Bubbles({
  ...
  */
  const middleIndex = numberOfPaddles%2? Math.floor(numberOfPaddles / 2) : numberOfPaddles / 2 - 0.5;
  ...
  const style = {
    ...
    width: `${diameter}vh`, //width of the bubble
    height: `${diameter}vh`, //height of the bubble
    ...
    */
    left: !toggleAnimation
      ? [`calc(50% - ${diameter * delta}vw - ${2 * delta}vw)`, "50%", `calc(50% + ${diameter * delta}vw + ${2 * delta}vw)`][
          positionType
        ]
      : "10%",
    ...
    */
    top: toggleAnimation
      ? `calc(40px + ${diameter * elm.position}vh + ${2 * elm.position}vh)`
      : "40px",
